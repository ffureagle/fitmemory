...
const EXTENSION_ZIP = path.resolve(ROOT_DIR, "download/FitMemory-uzanti.zip");
const EXTENSION_MANIFEST = path.resolve(ROOT_DIR, "extension/manifest.json");
const EXTENSION_VERSION = JSON.parse(readFileSync(EXTENSION_MANIFEST, "utf8")).version;

function extensionDownloadPage() {
  const zipName = `FitMemory-uzanti-${EXTENSION_VERSION}.zip`;
  return `<!doctype html>
<html lang="tr">
  <head>
    <meta charset="utf-8" />
    <meta name="viewport" content="width=device-width, initial-scale=1" />
    <title>FitMemory ${EXTENSION_VERSION} indir</title>
    <style>
      body { font-family: ui-sans-serif, system-ui, sans-serif; margin: 0; background: #111; color: #fff; }
      ...
      .ver { font-size: 22px; font-weight: 800; color: #ffe14a; margin: 0 0 12px; }
      a.btn {
        display: block;
        text-align: center;
        margin: 28px 0;
      ...
      ol { font-size: 16px; color: #ddd; line-height: 1.6; }
  ...
  <body>
    <main>
      <p>FitMemory</p>
      <p class="ver">Sürüm ${EXTENSION_VERSION}</p>
      <h1>Chrome uzantısını indir</h1>
      <p class="warn">chrome://extensions içindeki üst <strong>Güncelle</strong> tuşuna basma. <strong>Kaldır</strong> da basma — Kaldır hesabını ve dolabını siler. Zip’i <em>aynı</em> FitMemory-uzanti klasörünün üzerine çıkar, karttaki <strong>dairesel yenile</strong> ikonuna bas.</p>
      <a class="btn" id="download-btn" href="./FitMemory-uzanti.zip?v=${EXTENSION_VERSION}" download="${zipName}">${EXTENSION_VERSION} ZIP İNDİR</a>
      <p>Zip senin bilgisayarına iner. Masaüstündeki mevcut <code>FitMemory-uzanti</code> klasörünün <strong>üzerine</strong> çıkar. Klasörde <code>manifest.json</code> olacak ve içinde <code>"version": "${EXTENSION_VERSION}"</code> yazmalı.</p>
      <ol>
        <li>Chrome’da chrome://extensions aç, Geliştirici modunu aç</li>
        <li>Zip’i ilk yüklediğin <strong>aynı klasörün</strong> üzerine çıkar. Yeni klasör seçme — yeni klasör yeni kimlik verir ve hesabı unutturur</li>
        <li>FitMemory kartındaki <strong>dairesel yenile</strong> ikonuna bas (Kaldır / Güncelle değil)</li>
        <li>Kartta sürüm <strong>${EXTENSION_VERSION}</strong> görünmeli. Yan panelde de v${EXTENSION_VERSION} yazar; giriş açık kalmalı</li>
      </ol>
      <script>
        document.getElementById("download-btn").addEventListener("click", (event) => {
          const url = new URL("./FitMemory-uzanti.zip?v=${EXTENSION_VERSION}", window.location.href).href;
          try {
...
export async function createApp(options = {}) {
  ...
  app.get("/", (context) => context.html(extensionDownloadPage()));

  const sendExtensionZip = (context) => {
    if (!existsSync(EXTENSION_ZIP)) {
      return context.text("Zip henüz hazır değil.", 404);
    }
    const bytes = readFileSync(EXTENSION_ZIP);
    return context.body(bytes, 200, {
      "Content-Type": "application/zip",
      "Content-Disposition": `attachment; filename="FitMemory-uzanti-${EXTENSION_VERSION}.zip"`,
      "Content-Length": String(bytes.length),
      "Cache-Control": "no-store, no-cache, must-revalidate",
      "Pragma": "no-cache"
    });
  };
  app.get("/FitMemory-uzanti.zip", sendExtensionZip);
  app.get(`/FitMemory-uzanti-${EXTENSION_VERSION}.zip`, sendExtensionZip);
