...
export function agentResultToSnapshot(
  ...
  const snapshot: ProductSnapshot = {
    product: {
      ...fallback,
      brand: agent.brand || fallback.brand,
      name: agent.productName || fallback.name,
      fitLabel: agent.fitDescription || fallback.fitLabel,
    ...
    sizeChart: {
      found: headers.length > 1,
      title: "Doğrulanmış sunucu ajanı ürün ölçüleri",
      unit: "Centimeters",
      headers,
      rows: agent.sizeTable.map((row) => ({
        cells: [row.size, ...headers.slice(1).map((header) => row.measurements[header] ?? "")],
      })),
      rawText: "",
    },
