...
export function SessionProvider({ children }: PropsWithChildren) {
  ...
  const api = useMemo(() => new FitMemoryApi(apiBaseUrl), [apiBaseUrl]);
  ...
  const loadAccountData = useCallback(
    async (
      ...
      const [nextProfile, nextOrders, nextStyleBoard] = await Promise.all([
    ...
    void (async () => {
      try {
        ...
        const resolvedUrl = storedUrl
          ...
          : DEFAULT_API_URL;
        const currentApi = new FitMemoryApi(resolvedUrl);
        if (!active) return;
