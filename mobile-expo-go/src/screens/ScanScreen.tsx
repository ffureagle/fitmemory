...
export function ScanScreen({
  ...
  const updatePendingOrder = (clientKey: string, field: "productName" | "purchasedSize", value: string) => setPendingOrders((current) => current ? {
    ...current,
    orderCards: current.orderCards.map((card) => card.clientKey === clientKey ? { ...card, [field]: value } : card),
  } : current);
  const removePendingOrder = (clientKey: string) => setPendingOrders((current) => current ? {
    ...current,
          ...
          {pendingOrders ? (
            <View style={styles.orderReview}>
              <View style={styles.orderReviewHeader}>
                <View style={{ flex: 1 }}><Text style={styles.orderReviewEyebrow}>SİPARİŞ ONAYI</Text><Text style={styles.orderReviewTitle}>Bulunan: {pendingOrders.orderCards.length} ürün</Text></View>
                <Pressable onPress={() => setPendingOrders(null)} style={styles.orderReviewClose}><Text style={styles.orderReviewCloseText}>×</Text></Pressable>
              </View>
              <Text style={styles.orderReviewCopy}>Ürün adını ve satın aldığın bedeni kontrol et; gerekirse düzenle veya sil.</Text>
              <ScrollView style={styles.orderReviewList} keyboardShouldPersistTaps="handled">
                {pendingOrders.orderCards.map((card) => <View key={card.clientKey} style={styles.orderReviewCard}>
                  {card.imageUrl ? <Image source={{ uri: card.imageUrl }} style={styles.orderReviewImage} /> : null}
                  <View style={styles.orderReviewFields}><Text style={styles.orderReviewBrand}>{card.brand}</Text><TextInput value={card.productName} onChangeText={(value) => updatePendingOrder(card.clientKey, "productName", value)} style={styles.orderReviewInput} /><TextInput autoCapitalize="characters" value={card.purchasedSize} onChangeText={(value) => updatePendingOrder(card.clientKey, "purchasedSize", value.toUpperCase())} placeholder="Beden" style={styles.orderReviewSize} /></View>
                  <Pressable onPress={() => removePendingOrder(card.clientKey)} style={styles.orderReviewDelete}><Text style={styles.orderReviewDeleteText}>Sil</Text></Pressable>
                </View>)}
              </ScrollView>
